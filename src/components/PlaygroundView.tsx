...
export const PlaygroundView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Intro Banner */}
      <div className="mb-8 relative overflow-hidden p-6 rounded-2xl bg-gradient-to-br from-slate-900 via-sky-950/30 to-slate-900 border border-sky-500/20 shadow-xl animate-fade-in-up">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="text-xs text-slate-400 font-mono">
                Dual-Stage Guardrailed RAG Agent
              </span>
            </div>
            ...
            <p className="text-sm text-slate-300 mt-1 max-w-2xl leading-relaxed">
              Test incoming customer tweets against the full pipeline — intent classification,
              historical Kaggle RAG retrieval, deterministic policy escalation, and 280-char grounded reply generation.
            </p>
            {/* Pipeline step indicator */}
            <div className="mt-3 flex items-center flex-wrap gap-1.5 text-[10px] font-mono text-slate-500">
              {['P0 Safety Check','Intent Classify','RAG Retrieve','Escalation Policy','Reply Synthesize'].map((step, i, arr) => (
                <React.Fragment key={step}>
                  <span className="px-2 py-0.5 rounded bg-slate-800/60 border border-slate-700/60 text-slate-400">{step}</span>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className="px-3 py-2 text-xs font-medium text-slate-300 bg-slate-800/80 hover:bg-slate-800 rounded-lg border border-slate-700 hover:border-slate-600 transition flex items-center space-x-1"
            >
            ...
            <div className="space-y-6 animate-scale-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-sm relative overflow-hidden">
                  ...
                  <p className="text-lg font-bold text-white tracking-tight" style={{ fontFamily: 'Outfit, sans-serif' }}>
                    {result.intent.replace(/_/g, ' ')}
                  </p>
                  {/* Animated confidence bar */}
                  <div className="mt-2 h-1.5 rounded-full bg-slate-800 overflow-hidden">
                    <div
                      className="h-full rounded-full bg-gradient-to-r from-sky-500 to-indigo-500 progress-bar-animated"
                      style={{ width: `${(result.intentConfidence * 100).toFixed(0)}%` }}
                    />
                  </div>
